...
def conditioning(logprobs, cond_ids, model, past, ids_to_retain, weights):
    cond_logits = model(ids_to_retain[:, None], past=past)[0][:, -1]
    cond_logprobs = F.log_softmax(cond_logits, dim=-1)
    cond_logprobs = cond_logprobs[:, cond_ids] * weights[None, :]
    cond_logprobs = torch.mean(cond_logprobs, dim=-1)

    top_k = ids_to_retain.shape[0]
    ...
    logprobs[:, ids_to_retain] += cond_logprobs
...
def add_past(past, cond_past, weights):
